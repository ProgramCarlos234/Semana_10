...
st.set_page_config(page_title="Clasificador Inteligente", page_icon="👗", layout="wide")

st.title("🧠 Sistema Inteligente de Clasificación")

tab1, tab2, tab3 = st.tabs(["📄 Infografía", "👗 Ropa (Fashion MNIST)", "🔢 Números (MNIST)"])

with tab1:
    st.header("📄 Infografía del Proyecto")
...
with tab2:
    ...
    def get_fashion_model():
        ...
        if fashion_path.exists():
            return load_model(fashion_path)
    ...
    uploaded_file = st.file_uploader("Sube tu imagen de ropa", type=["png", "jpg", "jpeg", "bmp"], key="fashion")
    
    if uploaded_file is not None:
        image = Image.open(uploaded_file)
        st.image(image, caption="Imagen cargada", width=200)
        
        if st.button("Predecir Ropa", key="btn_fashion"):
            with st.spinner("Cargando modelos y clasificando..."):
                try:
                    ...
                    st.success(f"**Resultado:** {FASHION_CLASS_NAMES[pred_idx]}")
                    ...
                    for i, (idx, p) in enumerate(top):
                        st.write(f"{i+1}. {FASHION_CLASS_NAMES[idx]}: {p*100:.2f}%")
...
with tab3:
    ...
    uploaded_file = st.file_uploader("Sube tu imagen de número", type=["png", "jpg", "jpeg", "bmp"], key="mnist")
    
    if uploaded_file is not None:
        image = Image.open(uploaded_file)
        st.image(image, caption="Imagen cargada", width=200)
        
        if st.button("Predecir Número", key="btn_mnist"):
            with st.spinner("Cargando modelos y clasificando..."):
                try:
                    ...
                    st.success(f"**Resultado:** {MNIST_CLASS_NAMES[pred_idx]}")
                    st.info(f"**Confianza:** {probs[pred_idx] * 100:.2f}%")
                    
                    st.subheader("Top 3")
                    top = sorted(enumerate(probs), key=lambda x: x[1], reverse=True)[:3]
                    for i, (idx, p) in enumerate(top):
                        st.write(f"{i+1}. {MNIST_CLASS_NAMES[idx]}: {p*100:.2f}%")
                except Exception as e:
                    st.error(f"Error: {e}")
